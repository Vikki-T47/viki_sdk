...
class ForensicPDF(FPDF):
    def footer(self):
        ...
        self.cell(0, 10, f"Page {self.page_no()} | RSA Forensic Audit Standard v1.7", align="C")
    ...
    def generate_forensic_pdf(self, agent_name, case_data, output_path):
        ...
        for section in [("[01] VULNERABILITY ORIGIN", case_data['vulnerability_origin']),
            ...
            if "[02]" in section[0]:
                ...
                pdf.set_fill_color(255, 255, 255) # Сброс цвета
            else:
        ...
        print(f"📄 [v1.7] Forensic PDF created: {os.path.basename(output_path)}")